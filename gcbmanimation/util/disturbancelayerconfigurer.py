    ...
    *_moja.tif files along with their tiled metadata (disturbance type and year),
    ...
    def configure(self, study_area_path):
        ...
        layer_collection = LayerCollection(
            colorizer=self._colorizer, background_color=self._background_color
        )
        ...
        for layer in disturbance_layers:
            ...
            layer_attribute_table = json.load(open(layer_metadata_file, "rb")).get(
                "attributes"
            )
            ...
            disturbance_years = {
                attr["year"] for attr in layer_attribute_table.values()
            }
            for year in disturbance_years:
                ...
                layer_collection.append(
                    Layer(layer_tif, year, interpretation, Units.Blank)
                )
