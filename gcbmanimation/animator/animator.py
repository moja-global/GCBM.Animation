...
class Animator:
    ...
    def render(
        ...
        for indicator in self._indicators:
            ...
            indicator_legend_title = (
                f"{indicator.indicator} ({indicator.map_units.value[2]})"
            )
    ...
    def _render_single_view(
        ...
        quadrant_sizes = (
            ((70, 100), (30, 100), (0, 0), (0, 0))
            if legend
            else ((100, 100), (0, 0), (0, 0), (0, 0))
        )

        layout = QuadrantLayout(
            *quadrant_sizes, q1_scalebar=scalebar, q2_scalebar=False
        )
    ...
    def _create_animation(self, title, frames, fps=1):
        ...
        video_frames.append(
            video_frames[-1]
        )  # Duplicate the last frame to display longer.
        imageio.mimsave(
            os.path.join(self._output_path, f"{title}.wmv"),
            video_frames,
            fps=fps,
