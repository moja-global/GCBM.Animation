...
class BoundingBox(Layer):
    ...
    def min_pixel_bounds(self):
        ...
        if not self._min_pixel_bounds:
            ...
            for i, row in enumerate(raster_data):
                x_index = np.where(row != self.nodata_value)[
                    0
                ]  # First non-null value per row.
                if len(x_index) == 0:
                    continue
    ...
    def min_geographic_bounds(self):
        ...
        if not self._min_geographic_bounds:
            ...
            origin_x, x_size, _, origin_y, _, y_size, *_ = gdal.Open(
                self._path
            ).GetGeoTransform()
    ...
    def crop(self, layer):
        ...
        gdal.Warp(
            tmp_path,
            layer.path,
            dstSRS=self._get_srs(),
            creationOptions=gdal_creation_options,
            width=width,
            height=height,
            outputBounds=(
                self.info["cornerCoordinates"]["upperLeft"][0],
        ...
        cropped_layer = Layer(
            output_path, layer.year, layer.interpretation, layer.units
        )
