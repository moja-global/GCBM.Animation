...
class Layer:
    ...
    def info(self):
        ...
        if not self._info:
            self._info = json.loads(
                gdal.Info(
                    self._path, format="json", deserialize=False, computeMinMax=True
                ).replace("nan", "0")
            )
    ...
    def scale(self):
        ...
        if metres:
            ...
            pixel_size_m = distance(
                (origin_y, origin_x), (origin_y, origin_x + pixel_size)
            ).m
    ...
    def convert_units(self, units):
        ...
        if current_per_ha == new_per_ha:
            ...
            per_ha_modifier = (
                pixel_size_m2 / one_hectare if current_per_ha != new_per_ha else 1
            )
            simple_conversion_calc = " ".join(
                (
    ...
    def reclassify(self, new_interpretation, nodata_value=0):
        ...
        uninterpreted_values = np.isin(
            raster_data, list(self._interpretation.keys()), invert=True
        )
        ...
        collision_offset = (
            max(chain(self._interpretation.keys(), new_interpretation.keys())) + 1
        )
        ...
        for original_pixel_value, interpreted_value in self._interpretation.items():
            new_pixel_value = (
                inverse_new_interpretation[interpreted_value]
            ...
            if new_pixel_value == nodata_value:
                logging.info(
                    f"  No new pixel value for {interpreted_value}: setting to nodata ({nodata_value})"
                )

            raster_data[
                raster_data == original_pixel_value + collision_offset
            ] = new_pixel_value

        output_path = TempFileManager.mktmp(suffix=".tif")
        self._save_as(raster_data, nodata_value, output_path)
        reclassified_layer = Layer(
            output_path, self._year, new_interpretation, self._units
        )
    ...
    def flatten(self, flattened_value=1, preserve_units=False):
        ...
        flattened_layer = Layer(
            output_path, self.year, units=self._units if preserve_units else Units.Blank
        )
    ...
    def reproject(self, projection):
        ...
        reprojected_layer = Layer(
            output_path, self._year, self._interpretation, self._units
        )
    ...
    def _save_as(self, data, nodata_value, output_path):
        ...
        new_raster = driver.CreateCopy(
            output_path, original_raster, strict=0, options=gdal_creation_options
        )
